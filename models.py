...
class TranspalletEquipment(SQLModel, table=True):
    ...
    equipment_type: str = Field(default="caminhao", index=True)  # caminhao, moto, veicular
...
class TranspalletChecklist(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    employee_id: int = Field(foreign_key="employee.id", index=True)
    checklist_type: str = Field(default="caminhao", index=True)  # caminhao, moto, veicular
    equipment_code: str = Field(index=True)
    date: str = Field(index=True)  # YYYY-MM-DD
