# F?rias iniciando/terminando esta semana
    week_start = today_naive - timedelta(days=today_naive.weekday())
    week_end = week_start + timedelta(days=6)
    vacations_active_ids = set()
    
    for emp in employees:
        shift_label = emp.work_shift or "Turno"
        shift_short = shift_label[:3] if shift_label else "Turno"
        vac_start = normalize_date(emp.vacation_start)
        vac_end = normalize_date(emp.vacation_end)
        
        if vac_start and vac_end:
            if week_start <= vac_start <= week_end:
                alerts["ferias"].append({
                    "name": get_short_name(emp.name),
                    "status": f"Inicia {vac_start.strftime('%d/%m')}",
                    "shift": shift_short
                })
            elif week_start <= vac_end <= week_end:
                alerts["ferias"].append({
                    "name": get_short_name(emp.name),
            ...
            if vac_start <= today_naive <= vac_end:
                ...
                vacations_active_ids.add(emp.id)
    
    for emp in employees:
        if emp.status == 'vacation' and emp.id not in vacations_active_ids:
            shift_label = emp.work_shift or "Turno"
            shift_short = shift_label[:3] if shift_label else "Turno"
            vac_end = normalize_date(emp.vacation_end)
            days_left = None
            end_date = None
            if vac_end:
                delta = (vac_end - today_naive).days + 1
