    ...
    for emp in employees:
        if emp.vacation_start and emp.vacation_end:
            ...
            if v_e < check_start:
                ...
                print(f"ðŸâ€"️ Férias encerradas para {emp.name} - limpando dados de férias")
            ...
            if should_be_vacation:
                ...
                for shift in ["Manhã", "Tarde", "Noite"]:
                    ...
                    if not existing_routine:
                        ...
                        print(f"ðŸâ€"️ Criada rotina de férias para {emp.name} - {today_str} ({shift})")
...
def send_maintenance_email(report: dict, recipients: Optional[List[str]] = None) -> tuple:
    ...
    def _fix_mojibake(value) -> str:
        ...
        for _ in range(2):
            if not any(token in candidate for token in ("ÃÆâ€™", "Â", "â")):
                break
            try:
...
def migrate_existing_substitutions():
    ...
    with Session(engine) as session:
        ...
        if migrated > 0:
            session.commit()
            print(f"âÅ"… Migradas {migrated} substituições existentes para o histórico")
...
def ensure_pallet_count_schema():
    ...
    if "palletsector" not in existing_tables:
        with engine.begin() as conn:
            ...
            print("âÅ"… Tabela palletsector criada")
    ...
    if "palletcount" not in existing_tables:
        with engine.begin() as conn:
            ...
            print("âÅ"… Tabela palletcount criada")
    else:
        ...
        if "quantity" in existing_cols and "pallet_number" not in existing_cols:
            ...
            with engine.begin() as conn:
                ...
                conn.execute(text("CREATE INDEX IF NOT EXISTS ix_palletcount_shift ON palletcount (shift)"))
                conn.execute(text("CREATE INDEX IF NOT EXISTS ix_palletcount_status ON palletcount (status)"))
                print("âÅ"… Tabela palletcount recriada com nova estrutura")
    ...
    if "palletmaintenanceticket" not in existing_tables:
        with engine.begin() as conn:
            ...
            print("âÅ"… Tabela palletmaintenanceticket criada")
    ...
    if "palletcountemailrecipient" not in existing_tables:
        with engine.begin() as conn:
            ...
            print("âÅ"… Tabela palletcountemailrecipient criada")
            ...
            for col_name, col_type in missing.items():
                if col_name not in cols:
                    try:
                        ...
                        conn.commit()
                        logger.info(f"âÅ"… Coluna {col_name} adicionada à tabela client")
...
DELIVERY_RETURN_REASONS = {
    "COMERCIAL": [
        ...
        "CLIENTE NÃÆâ€™O FEZ PEDIDO",
        "PRAZO ERRADO",
        "PREÇO ERRADO",
        "SEM VASILHAME",
        "FORMA DE PAGAMENTO ERRADA",
        "VENDEDOR NÃÆâ€™O PASSOU",
        "TROCAS NÃÆâ€™O AUTORIZADAS",
        "TROCAS NÃÆâ€™O ENVIADAS",
    ],
    "MERCADO": [
        "HORÁRIO ENTREGA",
    ...
    "LOGÍSTICA": [
        ...
        "LOCAL ENTREGA NÃÆâ€™O LOCALIZADA",
        "ÁREA DE RISCO",
        "CAMINHÃÆâ€™O QUEBRADO NA ROTA",
        "FURTO / ROUBO",
        "QUANTIDADE ERRADA CARREGAMENTO",
        "PEDIDO NÃÆâ€™O ENTREGUE",
...
def _validate_delivery_assignment(
    ...
    logger.info(f"ðŸšâ€" Caminhões já vinculados ao motorista {employee_id}: {driver_plates}")
    ...
    logger.info(f"âÅ"… Validação passou: motorista {employee_id} pode usar caminhão {plate_norm}")
...
def _find_employee_by_driver_name(name: str, employees: List[models.Employee]) -> Optional[models.Employee]:
    ...
    for emp in employees:
        if _norm_text(emp.name) == target:
            ...
            break
    if exact:
        logger.debug(f"âÅ"… Motorista encontrado (exato): {exact.name}")
        return exact
    ...
    if best_emp and best_score >= 0.75:
        logger.debug(f"âÅ"… Motorista encontrado (similaridade {best_score:.0%}): {best_emp.name}")
        return best_emp
...
async def import_entregas_separacao(
    ...
    if not file.filename or not file.filename.lower().endswith((".xlsx", ".xls", ".csv")):
        ...
        logger.warning(f"âÅ' Arquivo inválido: {file.filename}")
    ...
    try:
        ...
        logger.info(f"ðŸâ€"ºï¸ Mapeamento de colunas: {col_map}")
        ...
        if missing_required:
            ...
            logger.error(f"âÅ' Colunas ausentes: {missing_required}")
            response = await separacao_page(request=request, date=date, shift=shift, session=session)
            response.context["delivery_import"] = import_result
            return response
        ...
        logger.info(f"ðŸšâ€" Motoristas cadastrados ({len(motoristas_cadastrados)}): {motoristas_cadastrados}")
        ...
        for idx, row in df.iterrows():
            ...
            if not emp:
                ...
                logger.warning(f"âÅ' Motorista não encontrado na linha {row_num}: '{driver_name_raw}'")
                continue
        ...
        logger.info(f"âÅ"… Importação concluída: {len(parsed_rows)} entregas criadas")
...
async def reassign_delivery_group(
    ...
    if not rows:
        logger.warning(f"âÅ' Nenhuma entrega encontrada para {source_name} na data {date}")
    ...
    if err:
        logger.error(f"âÅ' Validação falhou: {err}")
        feedback_encoded = urlencode({"delivery_feedback": err, "delivery_feedback_level": "error"})
        return RedirectResponse(url=f"/separacao?date={date}&shift={shift}&{feedback_encoded}", status_code=303)
    ...
    logger.info(f"âÅ"… Troca concluída: {len(rows)} entregas transferidas de {source_name} para {new_name}")
...
4. SUGEST̉ۢES DE DESENVOLVIMENTO (bullets)
    ...
    try:
        with Session(engine) as session:
            ...
            for shift, sector_list in shifts.items():
                 config_db = session.exec(select(models.SectorConfiguration).where(models.SectorConfiguration.shift_name == shift)).first()
                 if not config_db: 
                     logs.append(f"âÅ' Config não encontrada para turno '{shift}'")
                     continue
                 ...
                 if not data: 
                    logs.append(f"âÅ' JSON inválido/vazio para '{shift}'")
                    continue
                     ...
                     for cs in config_sectors:
                         ...
                         if db_name == json_name:
                             ...
                             if target != s.max_employees:
                                 logs.append(f"   ðŸâ€Â§ CORRIGINDO '{s.name}': {target} -> {s.max_employees}")
                                 cs['target'] = s.max_employees
                                 changed = True
                             else:
                                 logs.append(f"   âÅ"… '{s.name}' OK: {target}")
                             break
                 ...
                 if changed:
                     ...
                     session.add(config_db)
                     logs.append(f"ðŸâ€™Â¾ Salvando config para '{shift}'")
            
            session.commit()
            logs.append("âÅ"… Sync concluído com sucesso")
    except Exception as e:
        logs.append(f"âÅ' ERRO FATAL: {str(e)}")
        import traceback
...
async def update_sector(
    ...
    # print(f"ðŸâ€Â§ UPDATE_SECTOR CHAMADO: {sector_id} - {name}")
    ...
    try:
        ...
        print(f"âÅ' Erro ao salvar setor ou configuração: {e}")
        session.rollback()
        ...
        try:
            ...
            for days_back in range(1, MAX_DAYS_LOOKBACK + 1):
                ...
                if previous_allocations:
                    ...
                    print(f"âÅ"… Encontradas {len(previous_allocations)} alocações de {found_date_str}!")
                    break
                else:
            ...
            if previous_allocations and found_date_str:
                ...
                print(f"âÅ"… Escala copiada com sucesso! {len(allocations)} colaboradores alocados (origem: {found_date_str})")
            else:
                print(f"⚠️ Nenhuma alocação encontrada nos últimos {MAX_DAYS_LOOKBACK} dias para turno {shift}")
        except Exception as e:
            print(f"âÅ' Erro ao copiar escala de dias anteriores: {e}")
        ...
        try:
            ...
            for days_back in range(1, MAX_DAYS_LOOKBACK + 1):
                ...
                if previous_routines:
                    found_date_str = previous_date_str
                    print(f"âÅ"… Encontradas {len(previous_routines)} rotinas de {found_date_str}!")
                    break
                else:
            ...
            if previous_routines and found_date_str:
                ...
                if copied_count > 0:
                    session.commit()
                    print(f"âÅ"… {copied_count} rotinas persistentes copiadas de {found_date_str} (Férias/Afastado/Atestado)")
            ...
            print(f"âÅ' Erro ao copiar rotinas: {e}")
...
async def save_allocations(
    ...
    try:
        ...
        session.commit()
        print("âÅ"… Salvo com sucesso!")
        
        return {"success": True, "message": "Dados salvos com sucesso"}
        
    except Exception as e:
        error_msg = f"âÅ' ERRO ao salvar alocações: {e}"
        print(error_msg)
        import traceback
        traceback.print_exc()
...
async def set_employee_vacation(
    ...
    try:
        ...
        print(f"âÅ"… Férias definidas: {employee.name} - {vacation_start} até {vacation_end}")
        
        return {"success": True, "message": "Férias definidas com sucesso"}
    except Exception as e:
        print(f"âÅ' Erro ao definir férias: {e}")
        session.rollback()
        return JSONResponse({"error": str(e)}, status_code=500)
...
async def set_employee_routine_extended(
    ...
    try:
        ...
        print(f"âÅ"… Rotina estendida: {employee.name} - {routine} de {start_date_str} por {days} dias ({action_info})")
        ...
        print(f"âÅ' Erro ao criar rotina estendida: {e}")
        import traceback
        traceback.print_exc()
        session.rollback()
        return JSONResponse({"error": str(e), "success": False}, status_code=500)
...
async def set_employee_routine(
    ...
    try:
        ...
        print(f"âÅ"… Rotina atualizada: {employee.name} - {routine}")
        
        return {"success": True, "message": "Rotina atualizada com sucesso"}
    except Exception as e:
        print(f"âÅ' Erro ao atualizar rotina: {e}")
        session.rollback()
        return JSONResponse({"error": str(e)}, status_code=500)
...
async def routine_report(
    ...
    try:
        ...
        print(f"ðŸâ€Â DEBUG - Sectors from Sector table: {len(SECTORS)}")
        for s in SECTORS:
        ...
        print(f"ðŸâ€Â DEBUG - Setores no attendance_log: {unique_sectors}")
        print(f"ðŸâ€Â DEBUG - Total de colaboradores: {len(people_list)}")
...
async def return_employee_from_leave(
    ...
    print(f"âÅ"… {emp.name} retornou de {previous_status_label}. Rotinas: {routines_updated} atualizadas, {routines_created} dias criados")
