...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        ensure_checklist_type_schema()
        ensure_equipment_type_schema()
...
# --- Checklist Operacional por Tipo ---
CHECKLIST_PROFILES: Dict[str, dict] = {
    "caminhao": {
        "key": "caminhao",
        "label": "Caminhao",
        "items": [
    ...
    "moto": {
        ...
        "items": [
            {"key": "limpeza", "label": "Limpeza", "critical": False},
            {"key": "retrovisores", "label": "Retrovisores", "critical": True},
            {"key": "freio_dianteiro", "label": "Freio Dianteiro", "critical": True},
            {"key": "freio_traseiro", "label": "Freio Traseiro", "critical": True},
            {"key": "pneus", "label": "Pneus", "critical": True},
            {"key": "setas_farois", "label": "Setas e Farois", "critical": True},
            {"key": "buzina", "label": "Buzina", "critical": True},
            {"key": "oleo_motor", "label": "Nivel de Oleo", "critical": True},
            {"key": "corrente", "label": "Corrente/Transmissao", "critical": True},
            {"key": "documentacao", "label": "Documentacao", "critical": False}
        ]
    },
    "veicular": {
        "key": "veicular",
        ...
        "items": [
            ...
            {"key": "pneus", "label": "Pneus", "critical": True},
            {"key": "freios", "label": "Freios", "critical": True},
            {"key": "luzes", "label": "Luzes e Sinalizacao", "critical": True},
            {"key": "cinto_seguranca", "label": "Cinto de Seguranca", "critical": True},
            {"key": "extintor", "label": "Extintor", "critical": True},
            {"key": "triangulo", "label": "Triangulo", "critical": True},
            {"key": "oleo_agua", "label": "Nivel de Oleo e Agua", "critical": True},
            {"key": "avarias_lataria", "label": "Avarias de Lataria", "critical": False},
            {"key": "documentacao", "label": "Documentacao", "critical": False}
        ]
    }
}

# Legacy fallback (compatibilidade de chamadas antigas)
CHECKLIST_ITEMS = CHECKLIST_PROFILES["caminhao"]["items"]
...
def checklist_nonconforming_items(keys: Optional[List[str]], checklist_type: Optional[str] = None) -> List[dict]:
    label_map = checklist_item_label_map(checklist_type)
    critical_keys = checklist_critical_keys(checklist_type)
    items = []
    for key in (keys or []):
        items.append({
            "key": key,
...
def normalize_checklist_type(value: Optional[str]) -> str:
    key = (value or "").strip().lower()
    return key if key in CHECKLIST_PROFILES else "caminhao"

def checklist_profile_for(checklist_type: Optional[str]) -> dict:
    return CHECKLIST_PROFILES[normalize_checklist_type(checklist_type)]

def checklist_item_label_map(checklist_type: Optional[str] = None) -> dict:
    if checklist_type:
        profile = checklist_profile_for(checklist_type)
        return {item["key"]: item["label"] for item in profile["items"]}

    label_map = {}
    for profile in CHECKLIST_PROFILES.values():
        for item in profile["items"]:
            label_map[item["key"]] = item["label"]
    return label_map

def checklist_critical_keys(checklist_type: Optional[str] = None) -> set:
    profile = checklist_profile_for(checklist_type)
    return {item["key"] for item in profile["items"] if item["critical"]}

def infer_equipment_type_from_code(code: Optional[str]) -> str:
    code_norm = (code or "").strip().upper()
    if code_norm.startswith("MOT"):
        return "moto"
    if code_norm.startswith("VEI") or code_norm.startswith("CAR"):
        return "veicular"
    return "caminhao"

def resolve_equipment_type(equipment: Optional[models.TranspalletEquipment]) -> str:
    if not equipment:
        return "caminhao"
    explicit = (getattr(equipment, "equipment_type", "") or "").strip().lower()
    if explicit in CHECKLIST_PROFILES:
        return explicit
    return infer_equipment_type_from_code(getattr(equipment, "code", ""))

def parse_items_payload(raw_items: str, expected_keys: List[str]) -> dict:
    try:
...
def ensure_employee_access_schema():
    ...
    missing = {
        ...
        "mobile_access_separation": "BOOLEAN DEFAULT FALSE",
        "mobile_access_checklist": "BOOLEAN DEFAULT FALSE",
        "mobile_access_admin_start": "BOOLEAN DEFAULT FALSE"
    }
    added = set()
    with engine.begin() as conn:
        for col_name, col_type in missing.items():
        ...
        has_mobile_access = ("mobile_access" in existing) or ("mobile_access" in added)
        has_separation = ("mobile_access_separation" in existing) or ("mobile_access_separation" in added)
        has_checklist = ("mobile_access_checklist" in existing) or ("mobile_access_checklist" in added)
        has_admin_start = ("mobile_access_admin_start" in existing) or ("mobile_access_admin_start" in added)

        if has_separation and has_mobile_access:
            conn.execute(text(
                "UPDATE employee "
                "SET mobile_access_separation = COALESCE(mobile_access_separation, mobile_access, FALSE)"
            ))
        elif has_separation:
            conn.execute(text(
                "UPDATE employee "
                "SET mobile_access_separation = COALESCE(mobile_access_separation, FALSE)"
            ))

        if has_checklist:
            conn.execute(text(
                "UPDATE employee "
                "SET mobile_access_checklist = COALESCE(mobile_access_checklist, FALSE)"
            ))

        if has_mobile_access and has_separation and has_checklist:
            conn.execute(text(
                "UPDATE employee "
                "SET mobile_access = COALESCE("
                "mobile_access, "
                "(COALESCE(mobile_access_separation, FALSE) OR COALESCE(mobile_access_checklist, FALSE))"
                ")"
            ))
        elif has_mobile_access:
            conn.execute(text(
                "UPDATE employee "
                "SET mobile_access = COALESCE(mobile_access, FALSE)"
            ))

        if has_admin_start:
            conn.execute(text(
                "UPDATE employee "
                "SET mobile_access_admin_start = COALESCE(mobile_access_admin_start, FALSE)"
            ))
...
def ensure_checklist_type_schema():
    inspector = inspect(engine)
    table_name = getattr(models.TranspalletChecklist, "__tablename__", "transpalletchecklist")
    if table_name not in inspector.get_table_names():
        return
    existing = {col["name"] for col in inspector.get_columns(table_name)}
    with engine.begin() as conn:
        if "checklist_type" not in existing:
            conn.execute(text(f"ALTER TABLE {table_name} ADD COLUMN checklist_type TEXT"))
        conn.execute(text(
            f"UPDATE {table_name} "
            "SET checklist_type = COALESCE(NULLIF(checklist_type, ''), 'caminhao')"
        ))

def ensure_equipment_type_schema():
    inspector = inspect(engine)
    table_name = getattr(models.TranspalletEquipment, "__tablename__", "transpalletequipment")
    if table_name not in inspector.get_table_names():
        return
    existing = {col["name"] for col in inspector.get_columns(table_name)}
    with engine.begin() as conn:
        if "equipment_type" not in existing:
            conn.execute(text(f"ALTER TABLE {table_name} ADD COLUMN equipment_type TEXT"))
        conn.execute(text(
            f"UPDATE {table_name} "
            "SET equipment_type = COALESCE(NULLIF(equipment_type, ''), "
            "CASE "
            "WHEN UPPER(code) LIKE 'MOT%' THEN 'moto' "
            "WHEN UPPER(code) LIKE 'VEI%' OR UPPER(code) LIKE 'CAR%' THEN 'veicular' "
            "ELSE 'caminhao' END)"
        ))
...
def require_login(request: Request):
    ...
    if isinstance(user, dict) and user.get("type") == "employee":
        if not path.startswith("/mobile") and not path.startswith("/static") and not path.startswith("/api"):
            ...
            logger.warning("Access denied: mobile user %s tried to access %s", user.get("id"), path)
...
# --- Gamification V2 Engine ---
        ...
        modules = [
            ...
            },
            {
                ...
                "description": "Checklist diario por tipo de veiculo.",
                "icon": "check-square",
...
async def mobile_checklist_history(request: Request, session: Session = Depends(get_session)):
    ...
    checklist_type = normalize_checklist_type(request.query_params.get("type"))
    checklist_profile = checklist_profile_for(checklist_type)
...
async def mobile_checklist_page(request: Request, session: Session = Depends(get_session)):
    ...
    if checklist_type == "caminhao":
        checklist_query = checklist_query.where(
            or_(
                models.TranspalletChecklist.checklist_type == "caminhao",
                models.TranspalletChecklist.checklist_type == None
            )
        )
    else:
        checklist_query = checklist_query.where(models.TranspalletChecklist.checklist_type == checklist_type)

    checklists = session.exec(
        checklist_query.order_by(models.TranspalletChecklist.submitted_at.desc())
    ).all()
    ...
    if checklist_type == "caminhao":
        done_dates_query = done_dates_query.where(
            or_(
                models.TranspalletChecklist.checklist_type == "caminhao",
                models.TranspalletChecklist.checklist_type == None
            )
        ...
        done_dates_query = done_dates_query.where(models.TranspalletChecklist.checklist_type == checklist_type)
    done_dates = set(session.exec(done_dates_query).all())
    ...
    all_equipment = session.exec(select(models.TranspalletEquipment).order_by(models.TranspalletEquipment.code)).all()
    equipment_list = [eq for eq in all_equipment if resolve_equipment_type(eq) == checklist_type]
    preselected_equipment_code = (request.query_params.get("equipment") or request.query_params.get("vehicle") or "").strip().upper()
    available_codes = {eq.code.upper() for eq in equipment_list}
    if preselected_equipment_code and preselected_equipment_code not in available_codes:
        preselected_equipment_code = ""
    if not preselected_equipment_code and len(equipment_list) == 1:
        preselected_equipment_code = (equipment_list[0].code or "").strip().upper()

    return templates.TemplateResponse(
        "mobile/routine_checklist.html",
        {
            "equipment_list": equipment_list,
            "preselected_equipment_code": preselected_equipment_code,
            "checklist_type": checklist_type,
            "checklist_label": checklist_profile["label"],
            "checklist_type_tabs": [
                {
                    "key": profile["key"],
                    "label": profile["label"],
                    "href": f"/mobile/routine/checklist?type={profile['key']}",
                    "active": profile["key"] == checklist_type
                }
                for profile in CHECKLIST_PROFILES.values()
            ],
            "request": request,
            "employee": employee,
            "items": checklist_profile["items"],
            "today": today,
...
async def admin_checklists_dashboard(
    ...
    for checklist in checklists:
        for key in (checklist.nonconforming_keys or []):
            item_counter[key] += 1
    label_map = checklist_item_label_map(checklist.checklist_type)
    top_items = [
        {
            "key": key,
            "label": label_map.get(key, key),
...
async def admin_checklists_add_equipment(
    ...
    equipment_type_norm = normalize_checklist_type(equipment_type) if equipment_type else infer_equipment_type_from_code(code_norm)
    if not code_norm:
    ...
    equipment = models.TranspalletEquipment(
        code=code_norm,
        status="available",
        equipment_type=equipment_type_norm
    )
    session.add(equipment)
    session.commit()
...
async def admin_checklist_resend_email(
    ...
    # Montar relatÃ³rio
    label_map = checklist_item_label_map()
    nonconforming_items = checklist_nonconforming_items(checklist.nonconforming_keys, checklist.checklist_type)
...
async def api_create_checklist(
    request: Request,
    equipment_code: str = Form(...),
    checklist_type: str = Form("caminhao"),
    items: str = Form(...),
    observations: str = Form(""),
    ...
    checklist_type_norm = normalize_checklist_type(checklist_type)
    checklist_profile = checklist_profile_for(checklist_type_norm)
    checklist_item_keys = [item["key"] for item in checklist_profile["items"]]
    checklist_critical = checklist_critical_keys(checklist_type_norm)
    ...
    equipment_type_norm = resolve_equipment_type(equipment)
    if equipment_type_norm != checklist_type_norm:
        return JSONResponse(
            {"error": f"Equipamento do tipo '{equipment_type_norm}' nao corresponde ao checklist '{checklist_type_norm}'."},
            status_code=400
        )

    payload_items = parse_items_payload(items, checklist_item_keys)
    ...
    critical_flag = any(k in checklist_critical for k in nonconforming_keys)
    images = []
    if files:
    ...
    checklist = models.TranspalletChecklist(
        employee_id=employee_id,
        checklist_type=checklist_type_norm,
        equipment_code=equipment_code,
        date=date_val,
        shift=shift_val,
        status="submitted",
    ...
    if critical_flag:
        ...
        blocked_items = ", ".join([checklist_item_label_map(checklist_type_norm).get(k, k) for k in nonconforming_keys])
    ...
    if nonconforming_keys:
        report_items = checklist_nonconforming_items(nonconforming_keys, checklist_type_norm)
    ...
    if shift_ok:
        tx = models.GameXPTransaction(
            ...
            status="provisional",
            reason=f"Checklist {checklist_profile['label']} {equipment_code} | {date_val}",
            created_at=now_br
        )
        session.add(tx)
        session.commit()
...
async def api_list_checklists(
    ...
    if checklist_type:
        checklist_type_norm = normalize_checklist_type(checklist_type)
        if checklist_type_norm == "caminhao":
            query = query.where(
                or_(
                    models.TranspalletChecklist.checklist_type == "caminhao",
                    models.TranspalletChecklist.checklist_type == None
            ...
            query = query.where(models.TranspalletChecklist.checklist_type == checklist_type_norm)
    ...
    for checklist, employee in rows:
        result.append({
            ...
            "equipment_code": checklist.equipment_code,
            "checklist_type": checklist.checklist_type or "caminhao",
            "date": checklist.date,
            "shift": checklist.shift,
...
def send_pallet_missing_email(
    ...
    """
    Envia e-mail de alerta de paleteiras nÃ£o encontradas.
    Retorna (success: bool, error: str ou None)
    """
    smtp_port = parse_int_env(SMTP_PORT_RAW, 587)
    smtp_tls = parse_bool_env(SMTP_TLS_RAW, True)
    ...
    subject = f"ðŸš¨ ALERTA: {len(missing_pallets)} Paleteira(s) NÃ£o Encontrada(s) â€” {date_formatted}"
    ...
    <body style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 20px;">
        <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            <div style="background: linear-gradient(135deg, #dc2626, #991b1b); padding: 24px; text-align: center;">
                <h1 style="color: white; margin: 0; font-size: 24px;">ðŸš¨ Alerta de Paleteiras</h1>
                <p style="color: rgba(255,255,255,0.9); margin: 8px 0 0 0;">Contagem com divergÃªncias detectadas</p>
            </div>
            
            <div style="padding: 24px;">
                <p style="color: #374151; line-height: 1.6;">
                    A contagem de paleteiras realizada em <strong>{date_formatted}</strong> (Turno: <strong>{shift}</strong>) 
                    identificou as seguintes paleteiras como <strong style="color: #dc2626;">NÃƒO ENCONTRADAS</strong>:
                </p>
                
                <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                    <thead>
                        <tr style="background: #f1f5f9;">
                            <th style="padding: 12px 8px; text-align: left; font-weight: bold; color: #1e293b;">NÃºmero</th>
                            <th style="padding: 12px 8px; text-align: left; font-weight: bold; color: #1e293b;">Setor</th>
                            <th style="padding: 12px 8px; text-align: left; font-weight: bold; color: #1e293b;">Ãšltima Vez Vista</th>
                        </tr>
                    </thead>
                    <tbody>
                        {pallet_rows}
                ...
                <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 12px; margin: 20px 0; border-radius: 4px;">
                    <p style="color: #92400e; margin: 0; font-size: 14px;">
                        <strong>AÃ§Ã£o recomendada:</strong> Verifique se estas paleteiras foram enviadas para manutenÃ§Ã£o, 
                        transferidas para outro setor ou se hÃ¡ possibilidade de extravio.
                    </p>
                </div>
                ...
                <div style="background: #f1f5f9; padding: 12px; border-radius: 8px; margin-top: 20px;">
                    <p style="color: #64748b; margin: 0; font-size: 12px;">
                        ...
                        <strong>Data/Turno:</strong> {date_formatted} â€” {shift}
                    </p>
                </div>
            </div>
            ...
            <div style="background: #f8fafc; padding: 16px; text-align: center; border-top: 1px solid #e2e8f0;">
                <p style="color: #94a3b8; font-size: 11px; margin: 0;">
                    E-mail automÃ¡tico do Sistema de AnÃ¡lise Operacional
                </p>
            </div>
        </div>
    </body>
...
async def mobile_pallet_count(
    ...
    """PÃ¡gina mobile de contagem de paleteiras"""
    if not isinstance(current_user, dict):
        return RedirectResponse(url="/mobile/login", status_code=303)
...
async def api_register_pallet_count(
    ...
    if not isinstance(current_user, dict):
        return JSONResponse({"success": False, "error": "NÃ£o autorizado"}, status_code=401)
    
    user_id = current_user.get("id")
    employee = session.get(models.Employee, user_id)
    if not employee:
        return JSONResponse({"success": False, "error": "FuncionÃ¡rio nÃ£o encontrado"}, status_code=404)
    ...
    if not pallet_number:
        return JSONResponse({"success": False, "error": "NÃºmero da paleteira Ã© obrigatÃ³rio"}, status_code=400)
...
async def api_finalize_pallet_count(
    ...
    """Finaliza a contagem do turno e envia alertas de paleteiras nÃ£o encontradas"""
    if not isinstance(current_user, dict):
        return JSONResponse({"success": False, "error": "NÃ£o autorizado"}, status_code=401)
    ...
    return JSONResponse({
        "success": True,
        "message": f"Contagem finalizada. {len(missing_pallets)} paleteira(s) nÃ£o encontrada(s).",
...
async def api_create_pallet_maintenance_ticket(
    ...
    """Cria um chamado de manutenÃ§Ã£o para paleteira"""
    if not isinstance(current_user, dict):
        return JSONResponse({"success": False, "error": "NÃ£o autorizado"}, status_code=401)
    ...
    if not description:
        return JSONResponse({"success": False, "error": "DescriÃ§Ã£o do problema Ã© obrigatÃ³ria"}, status_code=400)
    ...
    if existing:
        ...
        existing.observations = f"ManutenÃ§Ã£o: {description}"
        session.add(existing)
    else:
        maintenance_count = models.PalletCount(
            ...
            employee_id=employee.id,
            status="maintenance",
            observations=f"ManutenÃ§Ã£o: {description}"
        )
...
async def admin_pallet_count_settings(
    ...
    """PÃ¡gina de configuraÃ§Ã£o do sistema de contagem de paleteiras"""
    message = request.query_params.get("message")
    level = request.query_params.get("level", "success")
...
async def admin_pallet_count_add_sector(
    ...
    name = name.strip()
    if not name:
        return pallet_count_settings_redirect("Nome do setor Ã© obrigatÃ³rio.", "error")
    ...
    if existing:
        ...
        return pallet_count_settings_redirect(f"Setor '{name}' jÃ¡ existe.", "error")
...
async def admin_pallet_count_edit_sector(
    ...
    if not sector:
        return pallet_count_settings_redirect("Setor nÃ£o encontrado.", "error")
...
async def admin_pallet_count_add_email(
    ...
    """Adiciona um e-mail de destinatÃ¡rio"""
    email_normalized = normalize_email(email)
    if not email_normalized:
        return pallet_count_settings_redirect("E-mail invÃ¡lido.", "error")
    ...
    if existing:
        if existing.is_active:
            return pallet_count_settings_redirect("E-mail jÃ¡ cadastrado.", "error")
        else:
            existing.is_active = True
            existing.name = name.strip() if name else existing.name
...
async def admin_pallet_count_remove_email(
    ...
    """Remove um e-mail de destinatÃ¡rio"""
    recipient = session.get(models.PalletCountEmailRecipient, recipient_id)
    if not recipient:
        return pallet_count_settings_redirect("E-mail nÃ£o encontrado.", "error")
...
async def admin_pallet_count_close_ticket(
    ...
    """Fecha um chamado de manutenÃ§Ã£o"""
    ticket = session.get(models.PalletMaintenanceTicket, ticket_id)
    if not ticket:
        return pallet_count_settings_redirect("Chamado nÃ£o encontrado.", "error")
...
# (SubstituÃ­do pelo sistema de contagem por nÃºmero individual em /mobile/pallet-count)
# ============================================================================
# As rotas abaixo foram removidas pois o modelo foi alterado para usar pallet_number
# ao invÃ©s de quantity:
