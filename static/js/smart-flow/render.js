...
const Render = {
            ...
            <div class="space-y-4">
                ...
                <div class="pt-2 border-t border-slate-700/50">
                    ...
                    <div class="grid grid-cols-1 gap-2 sm:grid-cols-3 mb-3">
                            ...
                            ${this.renderRoutineButton(employee.id, 'present', 'emerald', '🙌', 'Presente', { compact: true, emphasis: 'large' })}

                            ${this.renderRoutineButton(employee.id, 'absent', 'red', '✗', 'Falta', { compact: true, emphasis: 'large' })}

                            ${this.renderRoutineButton(employee.id, 'sick', 'orange', '🏥', 'Atestado', { compact: true, emphasis: 'large' })}

                    </div>

                    <div class="grid grid-cols-2 gap-2 md:grid-cols-4">

                            ${this.renderRoutineButton(employee.id, 'dayoff', 'slate', '📅', 'Folga', { compact: true })}

                            ${this.renderRoutineButton(employee.id, 'vacation', 'amber', '🏖️', 'Férias', { compact: true })}

                        ${this.renderRoutineButton(employee.id, 'away', 'indigo', '🚫', 'Afastado', { compact: true })}
                        ${this.renderRoutineButton(employee.id, 'early_exit', 'rose', '⏰', 'Saída antecipada', { compact: true })}
    ...
    renderRoutineButton(empId, routineKey, color, icon, label, options = {}) {

        const { compact = false, emphasis = 'normal', extraClasses = '' } = options;

        let sizeClass = compact ? 'p-2.5 min-h-[60px]' : 'p-4 min-h-[80px]';

        let iconClass = compact ? 'text-xl mb-0.5' : 'text-2xl mb-1';

        let textClass = compact ? 'text-[9px]' : 'text-xs';



        if (emphasis === 'large') {

            sizeClass = 'p-3.5 min-h-[78px] md:min-h-[96px]';

            iconClass = 'text-3xl mb-1 md:text-[2.5rem]';

            textClass = 'text-xs md:text-[11px]';
        ...
        return `
            ...
            <button onclick="Render.openRoutineForm(${empId}, '${routineKey}', '${label}')"
                ...
                class="flex flex-col items-center justify-center ${sizeClass} w-full h-full bg-slate-700/30 hover:bg-${color}-600/20 border border-slate-600/50 hover:border-${color}-500/50 rounded-xl transition-all active:scale-95 group ${extraClasses}">
